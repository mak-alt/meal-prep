...
@section('content')
    <div class="content-wrapper">
        {{ \Diglactic\Breadcrumbs\Breadcrumbs::render('backend.addons.edit', $addon) }}
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card card-primary">
                            <form action="{{ route('backend.addons.update', $addon->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="card-body">
                                        ...
                                        <div class="col-lg-6">
                                            <div class="form-group">
                                                ...
                                                <input type="number" class="form-control" id="required-meals-amount"
                                                       ...
                                                       value="{{ old('price', $addon->required_minimum_meals_amount) }}"
                                                       placeholder="Required meals amount..."
                                                       min="1" required>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="select2" autocomplete="off" name="category_ids[]"
                                                    ...
                                                    @foreach($categories as $category)
                                                        <option
                                                            value="{{ $category->id }}" {{ in_array($category->id, old('category_ids', $addon->categories->pluck('id')->toArray())) ? 'selected' : '' }}>
                                                            {{ $category->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                ...
                                                <div class="form-group_inner">
                                                    <select name="meal_ids[]" class="select2 form-control"
                                                        ...
                                                        @foreach($meals as $meal)
                                                            <option
                                                                value="{{ $meal->id }}" {{ in_array($meal->id, old('meal_ids', $addon->meals->pluck('id')->toArray())) ? 'selected' : '' }}>
                                                                {{ $meal->name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
